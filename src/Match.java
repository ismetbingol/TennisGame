...
public class Match {
    ...
    private void givePointForLeftHand(Player playerOne, Player playerTwo) {
        if (playerOne.getHand() == LEFT) {
            playerOne.addExperience(LEFT_HAND_POINT);
        }
        if (playerTwo.getHand() == LEFT) {
            playerTwo.addExperience(LEFT_HAND_POINT);
        }
    }
